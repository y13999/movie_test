...
for b in range(number001):
    ...
    # 将文件分割为多个单词的格式
    # 定义分隔符为.全部转换为空格
    table001=str.maketrans('.', ' ')
    newlistname=filelist001[b].translate(table001).split(' ')
    mark001=1
    movie_name001 = ''
    movie_years001 = ''
    movie_size001 = ''
    for c in newlistname:
        if c in movie_size_difind:
            movie_size001=c
        if int_it(c) in range(1900,2999):
            movie_years001=c
            mark001=0
        elif mark001==1:
            movie_name001=movie_name001+c+' '
            mark001=1
    file_all[b][2]=movie_name001[:len(movie_name001)-1]
    file_all[b][3]=movie_years001
    file_all[b][4]=movie_size001

    aa=get_douban_html(file_all[b][2])
    ...
    file_all[b][5]= get_douban_chinese_name(douban_name_key, aa)
    file_all[b][6]= get_douban_origin_name(douban_ori_name_key, aa)
    file_all[b][7]=get_douban_point(douban_point_key, aa)
    if isAllEnCh(file_all[b][6])==True:
        bb = get_imdb_html(file_all[b][6])
        file_all[b][8]=get_imdb_point(imdb_point_key, bb)
    new_doc_name = ''
